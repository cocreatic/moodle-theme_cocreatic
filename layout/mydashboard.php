...
/**
 * A two column layout for the cocreatic theme.
 *
...
if (is_siteadmin()) {
    ...
    // Get the disk usage.
    $cache = cache::make('theme_cocreatic', 'admininfos');
...
theme_cocreatic_boostnavigation_extend_navigation($PAGE->navigation);

$templatecontext['flatnavigation'] = $PAGE->flatnav;

$profile_class = 'profile_guest';
if (property_exists($USER, 'profile') && is_array($USER->profile) && array_key_exists('relacion', $USER->profile)) {
    $profile_class = 'profile_' . $USER->profile['relacion'];
}
$templatecontext['profileclass'] = $profile_class;

echo $OUTPUT->render_from_template('theme_cocreatic/mydashboard', $templatecontext);
