 ...
 * @return string
 */
function theme_cocreatic_get_extra_scss($theme) {
    $scss = $theme->settings->scss;

    $scss .= theme_cocreatic_set_bgsimg($theme);
 ...
 * @return string
 */
function theme_cocreatic_set_bgsimg($theme) {
    global $OUTPUT;
 ...
 * @return string
 */
function theme_cocreatic_get_main_scss_content($theme) {
    global $CFG;

    $scss = file_get_contents($CFG->dirroot . '/theme/boost/scss/preset/default.scss');

    // CocreaTIC scss.
    $cocreaticvariables = file_get_contents($CFG->dirroot . '/theme/cocreatic/scss/cocreatic/_variables.scss');
    $cocreatic = file_get_contents($CFG->dirroot . '/theme/cocreatic/scss/cocreatic.scss');

    // Combine them together.
    return $cocreaticvariables . "\n" . $scss . "\n" . $cocreatic;
}
 ...
 * @return string
 */
function theme_cocreatic_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 * @return mixed
 */
function theme_cocreatic_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
 ...
 * @return string
 */
function theme_cocreatic_get_setting($setting, $format = false) {
    $theme = theme_config::load('cocreatic');
 ...
 */
function theme_cocreatic_boostnavigation_extend_navigation(global_navigation $navigation) {
    global $CFG;
    ...
    if ($mycoursesnode = $navigation->find('mycourses', global_navigation::TYPE_ROOTNODE)) {
        ...
        foreach ($mycourseschildrennodeskeys as $k) {
            ...
            if ($CFG->navshowmycoursecategories) {
                ...
                $allchildrennodes = theme_cocreatic_boostnavigation_get_all_childrenkeys($mycoursesnode->get($k));
 ...
 * @return array
 */
function theme_cocreatic_boostnavigation_get_all_childrenkeys(navigation_node $navigationnode) {
    ...
    foreach ($childrennodeskeys as $ck) {
        $allchildren = array_merge($allchildren, theme_cocreatic_boostnavigation_get_all_childrenkeys($navigationnode->get($ck)));
    }
